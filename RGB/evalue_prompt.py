...
def prompt_wrapper(prompt_list, query, docs, model, system, instruction):
    if len(docs) == 0:
        ...
        elif args.type in ["ziya_reader", "ziya_writer", "ziya2", "ziya_reader+"]:
        ...
        if args.type.startswith("ziya_reader"):
        ...
        if args.type == "baichuan" :
            ...
            prompt = chatglm2_prompt(text)
        elif args.type.startswith("ziya"):
            prompt = ziya_prompt(text)
        elif args.type in ["qwen_7b", "qwen_14b", "qwen_72b"]:
            prompt = qwen_prompt(text)
        elif args.type == "intern":
...
def predict(prompt, ground_truths, temperature, dataset, sampling_params):
    ...
    predictions = model.generate(prompt, sampling_params, use_tqdm=True)
    
    labels = []
    factlabels = []
    predictions_list = []
    for prediction, ground_truth in zip(predictions, ground_truths):
        prediction = prediction.outputs[0].text
        if 'zh' in dataset:
        ...
        if '信息不足' in prediction or 'insufficient information' in prediction:
            ...
            labels.append(checkanswer(prediction, ground_truth))
    ...
    return labels, predictions_list, factlabels
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_num', type=int, help='number of gpu')
    ...
    parser.add_argument(
        '--factchecking', type=str2bool, default=False,
    ...
    with open(filename,'w') as f:
        for instance in tqdm.tqdm(instances):
            ...
            try:
                ...
                if passage_num == 0:
                    ...
                    prompt_list = prompt_wrapper(prompt_list, query, docs, model, system, instruction)
        ...
        labels,predictions,factlabels = predict(prompt_list, answer_list, temperature, args.dataset, sampling_params)
        for instance, label, prediction, factlabel, ans, query, docs in zip(instances, labels,predictions,factlabels, answer_list,query_list, docs_list):
