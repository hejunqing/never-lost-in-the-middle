...
def get_pred(model, tokenizer, data, max_length, max_gen, prompt_format, dataset, device,model_path):
    ...
    for json_obj in tqdm(data):
        ...
        print("len(tokenized_prompt)_1", len(tokenized_prompt))
    ...
    for i, prompt in enumerate(prompts):
        response, history = model.chat(tokenizer, prompt, temperature=0.2, history=None)
        ...
        tokenized_prompt = tokenizer(prompt, truncation=False, return_tensors="pt").input_ids[0] 
        print("len_2", len(tokenized_prompt))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path',default='/cognitive_comp/liuyibo/cg/pretrain_model/Qwen-14B-Chat',type=str)
    ...
    parser.add_argument('--out_dir',default='pred_Qwen-14B-Chat_retrieval_single',type=str)
    ...
    if 'chatglm' in args.model_path:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_path,trust_remote_code=True)
        model = AutoModelForCausalLM.from_pretrained(args.model_path, device_map="auto", torch_dtype=torch.bfloat16,trust_remote_code=True).to(device)
        #
    model = model.eval()
    # define max_length
